...
- `supertype(Class)` dropped a generic outer class's arguments when projecting onto an inner class: `Outer<String>.Inner<Integer>` came back as `Outer.Inner<Integer>`, unequal to the captured form and useless as a cache key. Only `parameterized` has that limitation now.
