...
- Interoperability tests and a guide covering Gson, Jackson and Guava. No adapter modules: the hand-off between Specular and every one of those libraries is `java.lang.reflect.Type`, so a conversion is already a single call. What needed proving is that the types Specular *fabricates* — parameterized types, generic arrays and wildcards it builds rather than reflects — are interpreted correctly by each of them, since a library that quietly misses a type argument deserializes into the wrong shape instead of failing. Verified against Gson 2.14.0, Jackson 3.2.2 and Guava 33.6.0-jre, which are test-scope dependencies; the only runtime dependency is still Commons Lang.
- Documented one asymmetry the tests found: Jackson's `JavaType` implements `Type`, so `TypeRef.of` accepts one and returns a reference that cannot answer anything about it. Converting *to* Jackson is fine; coming back is not. Gson and Guava round-trip to an equal reference.
...
- `TypeRef.coerced()` — asserts a type reflection cannot know. `TypeRef<String> name = TypeRef.fieldType(field).coerced();` The library cannot prove what a field holds, but the caller often can; this makes that claim explicit and greppable instead of an unchecked cast at the call site. Where the compiler *can* establish the type, `where` proves it instead.
...
- **A type nested more than 32 levels deep was silently returned half-substituted.** The substitution walker's recursion guard counted structural descent — every type argument, array component and wildcard bound — when the only thing that can actually loop is a chain of variable bindings. A legal, deeply-nested type hit the limit and came back with its variables still in it, and `where` would "succeed" with only a later `resolved()` noticing. The guard is now a set of the variables currently being expanded, so no legitimate depth can trip it, and a genuine cycle throws `IllegalStateException` rather than producing a wrong type.
- Substituting a wildcard's bound with another wildcard kept only the first bound of an intersection, and left illegal types such as `? extends ? super Integer` behind. A nested wildcard now contributes all of its bounds from the matching side, which widens those cases to plain `?` rather than inventing a bound that was never there.
- `supertype(Class)` dropped a generic outer class's arguments when projecting onto an inner class: `Outer<String>.Inner<Integer>` came back as `Outer.Inner<Integer>`, unequal to the captured form and useless as a cache key. Enclosing classes resolve at every level, so `Outer<String>.Mid<Integer>.Deep<Long>` keeps all three. Only `parameterized` has that limitation now.
- `where`'s rejection message named a type variable without saying what declared it, so two variables that share a name produced "`List<? extends T>` has no type variable T to substitute (it has T)". Variables are now described with their declaration.
...
[Unreleased]: https://github.com/jwcarman/specular/compare/0.4.0...HEAD
[0.4.0]: https://github.com/jwcarman/specular/releases/tag/0.4.0
